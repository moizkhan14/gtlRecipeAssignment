 ...
 * ```
 *
 * All coroutines started on [CoroutineScopeRule] must complete (including timeouts) before the test
 ...
 * ```
 *
 * You may call [DelayController] methods on [CoroutineScopeRule] and they will control the
 * virtual-clock.
 *
 * ```
 ...
 * ```
 *
 * By default, [CoroutineScopeRule] will be in a *resumed* state.
 *
 * @param dispatcher if provided, this [TestCoroutineDispatcher] will be used.
 */
@ExperimentalCoroutinesApi
class CoroutineScopeRule(val dispatcher: TestCoroutineDispatcher = TestCoroutineDispatcher()) :
        TestWatcher(),
